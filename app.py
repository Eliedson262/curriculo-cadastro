...
class Curriculo(db.Model):
    ...
    tipo              = db.Column(db.String(50))   # 'Criação' ou 'Envio existente'
    # dados pessoais
    ...
    pretensao         = db.Column(db.String(50))
    # conteúdo
    objetivo          = db.Column(db.Text)
    escolaridade      = db.Column(db.String(100))
    curso_grad        = db.Column(db.String(150))
    instituicao       = db.Column(db.String(150))
    ano_conclusao     = db.Column(db.String(20))
    hard_skills       = db.Column(db.Text)
    soft_skills       = db.Column(db.Text)
    certificacoes     = db.Column(db.Text)
    projetos          = db.Column(db.Text)
    experiencia       = db.Column(db.String(50))   # 'Sim' ou 'Não'
    observacoes       = db.Column(db.Text)
    # arquivos
    foto_base64       = db.Column(db.Text)
    foto_mime         = db.Column(db.String(50))
    foto_nome         = db.Column(db.String(200))
    cv_base64         = db.Column(db.Text)
    cv_mime           = db.Column(db.String(50))
    cv_nome           = db.Column(db.String(200))
    # controle
    template_escolhido = db.Column(db.String(20))
    pago              = db.Column(db.Boolean, default=False)
    pagamento_id      = db.Column(db.String(150))
...
@app.route('/')
def index():
    return render_template('index_atualizado.html')
